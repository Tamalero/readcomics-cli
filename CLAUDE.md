...
Built by `build-windows.ps1`. Run on a Windows machine:

```powershell
powershell -ExecutionPolicy Bypass -File build-windows.ps1
```

**Requirements:** Python 3.10+ in PATH (from python.org). Everything else (PySide6, Playwright, Firefox, PyInstaller) is downloaded automatically by the script.

**Output:** `dist\ReadComics\` — a self-contained folder (~400 MB) with:
- `ReadComics.exe` — main executable (no console window)
- `browsers\` — bundled Playwright Firefox
- `_internal\` — Python runtime + all packages

**Distribute:** zip the entire `dist\ReadComics\` folder. Users just unzip and run `ReadComics.exe` — no Python or browser install needed.

**Key implementation details:**
- `readcomics.spec` — PyInstaller one-dir spec; bundles `playwright/driver/` explicitly (contains the self-contained Node.js runtime Playwright uses to manage browsers)
- `gui.py` top-of-file frozen detection sets `PLAYWRIGHT_BROWSERS_PATH` to `<exe dir>\browsers` before any import, only when `sys.frozen == True`
- UPX disabled in spec (`upx=False`) to avoid antivirus false positives
- `console=False` so no black terminal window appears on launch
- Icon generated by Pillow (Catppuccin Mocha design) and saved as `docs/icon.ico` during build
