...
`.github/workflows/release-windows.yml` triggers on every published release and on `workflow_dispatch`. It builds on `windows-latest`, zips `dist\ReadComics\`, and uploads `ReadComics-Windows.zip` to the release automatically (~15 min after release is published).

To trigger manually for an existing release:
```sh
gh workflow run release-windows.yml --ref master --field tag=vX.Y.Z
```

### Manual (on a Windows machine)

Requires only Python 3.10+ in PATH:
```powershell
powershell -ExecutionPolicy Bypass -File build-windows.ps1
```
Zip `dist\ReadComics\` and distribute — fully self-contained (~400 MB).

**Output folder structure:**
```
dist\ReadComics\
├── ReadComics.exe   (no console window)
├── browsers\        (Playwright Firefox)
└── _internal\       (Python runtime + packages)
```

**Key implementation details:**
- `build-icon.py` — standalone Pillow icon generator; called by both `build-windows.ps1` and CI. If Pillow fails, exits cleanly with a warning (no empty `.ico` written — spec handles missing icon gracefully via `_icon = None`)
- `readcomics.spec` — PyInstaller one-dir spec; bundles `playwright/driver/` explicitly (self-contained Node.js runtime Playwright uses); `upx=False` (antivirus); `console=False`
- `gui.py` top-of-file sets `PLAYWRIGHT_BROWSERS_PATH = <exe_dir>\browsers` before any import when `sys.frozen == True`
- **Bug fixed in `build-windows.ps1`:** original used bash `<<'HEREDOC'` syntax for Python icon code — not valid PowerShell. Extracted to `build-icon.py` instead.
...
```sh
# 1. Bump VERSION file and update CLAUDE.md version line
# 2. Commit + push
git add VERSION CLAUDE.md && git commit -m "chore: bump version to X.Y.Z" && git push origin master
# 3. Build AppImage (Linux)
bash build-appimage.sh
# 4. Tag + push tag
git tag vX.Y.Z && git push origin vX.Y.Z
# 5. Create GitHub release with Linux artifacts
gh release create vX.Y.Z ReadComics-x86_64.AppImage ReadComics-x86_64.AppImage.zsync \
  --title "ReadComics vX.Y.Z" --notes "..."
# 6. Windows zip built and attached automatically by GitHub Actions (~15 min)
